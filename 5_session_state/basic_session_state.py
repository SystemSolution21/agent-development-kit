...
# from root_agent.agent import question_answer_agent
...
async def main() -> None:
    ...
    # Create input event loop in separate thread preventing from blocking the async main event loop
    input_loop = asyncio.get_running_loop()

    while True:
        try:
            input_text: str = await input_loop.run_in_executor(
                None,
                input,
                "Ask a question (Enter 'q' to quit or 'Crtl + c' to exit): ",
            )
